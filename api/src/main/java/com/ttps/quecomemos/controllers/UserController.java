...
public class UserController {
  ...
  @ApiResponses({
      ...
      @ApiResponse(responseCode = "500", description = "Unexpected error occurred", content = @Content) })
  public ResponseEntity<ApiResponseDTO<UserWithoutPasswordDTO>> login(@RequestBody
  LoginUserDTO loginUserDTO) {
    ...
    UserWithoutPasswordDTO loggedUser = userService.authenticateUser(loginUserDTO);

    String token = jwtService.generateToken(loggedUser.getDni());

    ApiResponseDTO<UserWithoutPasswordDTO> response = new ApiResponseDTO<>(loggedUser,
  ...
  @ApiResponses({
      ...
      @ApiResponse(responseCode = "401", description = "Invalid credentials", content = @Content),
      @ApiResponse(responseCode = "500", description = "Unexpected error occurred", content = @Content) })
  public ResponseEntity<ApiResponseDTO<UserWithoutPasswordDTO>> updateClient(@PathVariable
  ...
  UpdateClientDTO updateClientDTO) {
    ...
    UserWithoutPasswordDTO userWithoutPasswordDTO = new UserWithoutPasswordDTO(
        updatedUser.getId(), updatedUser.getDni(), updatedUser.getName(),
        updatedUser.getEmail(), updatedUser.getRole());

    ApiResponseDTO<UserWithoutPasswordDTO> response = new ApiResponseDTO<>(
        userWithoutPasswordDTO, "Client updated successfully", HttpStatus.OK);
